...
| 18 | **The zoom's upward rung is built, verified, and ~~deliberately unwired~~ WIRED (narrowed 2026-07-23).** The camera-driven trigger landed: `crate::site` derives the view-necessity threshold (one coarse particle's matter share against the docs/49 angular budget, `d* = s/theta`, measured from the live field when one exists), and the space band materializes the Ground Zero world's declared ball and strata patch through this rung on the descending crossing, folds through the docs/61 gauge on the ascending one, with the ledger and every refusal on the HUD. What stays OPEN, measured by the first consumer: (a) a RELIEF surface stalls the relax at ~5e-2 (an order over the release bound; flat and guarded free-surface lattices release fine), so the shipped site is the EXACT split with the residual stated, and the deferred computation is a fringe-corrected density estimate in the rung, closing test: the relief stall test flipping from expected-refusal to release under the bound; (b) an ISOLATED sub-resolution body (the ball) has no coarse environment to relax against (measured divergent), so it splits exactly, unrelaxed, stated; (c) NARROWED 2026-07-23: the mid-event hand-down landed as BOUNDARY STATE - the guard band re-samples the coarse field once per coarse step (`site::resample_guards`; the guards ARE the coarse field at the boundary, so the event's energy arrives through state, not a formula), `site::EventWindow` books the window within its stated 1e-6 bound, and the pi-scaling gate has its end-to-end consumer (`refine::measure_crater_rim` at the field's own quantum, prediction frozen from the measured contact state, verdict or stated sub-quantum refusal on the HUD). Still open in (c): any entry of the fine patch into dynamics (the release gate stands) and the N-material upload reaching the guards (the sampled state inherits the coarse field's EOS collapse, the named IOU). `crate::refine` is docs/59 item 3, the conserved hand-down: one-shot icosahedral splitting with a mandatory retained center child (stencil constants re-derived by least squares for the engine's OWN kernel, separation 0.3051 h and child smoothing 0.7915 h in the support-radius convention; the literature pair 0.2 h / 0.9 h measures 4.9% L2 density error on this kernel vs 0.7% for the derived pair), relax-then-release against the coarse field's interpolated density with a frozen clock (release is the stated 5e-3 relative density-error bound, never an iteration count), one-rung-per-interface discipline and contamination as refusal, the five-quantity conservation ledger, and the Holsapple-Housen pi-scaling gate (v2.2.1 rows) with its live crater consumer | native tests: exact conservation of mass, momentum, angular momentum, kinetic and internal energy on the split; density blip measured 7.5e-2 released to 5.0e-3 on uniform basalt and 9.5e-2 to 5.0e-3 across a basalt/iron interface; contamination and interface-ratio refusals with stated reasons; the gate reproduces a hand-computed Meteor Crater example; the window book is pinned by a quiet-hot-quiet pulse across the shipped site's boundary (guards carry exactly the field's state, fine children bitwise untouched, mass single-owner) and the rim measurement by synthetic bowls (one-quantum dimple refuses, intact shell refuses). Green whether or not anything calls it, which is exactly why this row exists (§7) | **docs/59** (order-of-work items 2 and 3) | `crate::site` (the trigger, the spec, the hand-down, the fold) and `OrbitDemo::update_site` (the per-frame crossing check, the HUD line, the billboard draw); armed by any world carrying a `ground` block (`worlds/ground-zero/world.json`). The row stays open for (a)-(c) |
