...
**What.** Two additions to `gpu_sph` (docs/58 items 4/5/7), merged from the collision-unify
...
`omega = (0, 0, spin)`), so the existing geometry tests pass unchanged. The only merge conflict
was the tail of the test module, where main had appended the particalized far-apart staging
test; all tests from both sides are kept.
...
a +z-only path could never produce). Full native suite 347/347 green;
